...
class RoleService {
    ...
    def getRolePermIdList(Role roleInstance) {
        if (!roleInstance) {
            return []
        }
        springSecurityService.findPremsByRole(roleInstance.authority).id ?: []
    }

    def getAllPrem() {
        PermissionGroup.listOrderById()
    }

    def assignPerm(Role role, perms) {
        springSecurityService.assignPerm(role, perms)
    }
